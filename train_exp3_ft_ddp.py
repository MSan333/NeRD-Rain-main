"""
train_exp3_ft_ddp.py — 基于 DDP_4GPU_exp3_lr2e5/model_best.pth 的双卡微调

修改点：
  - session: DDP_2GPU_exp3_lr1e5_ft
  - start_lr: 1e-5
  - end_lr:   1e-6
  - warmup:   关闭
  - DDP 2卡（GPU 0,3）
...
parser.add_argument('--session', default='DDP_2GPU_exp3_lr1e5_ft', type=str, help='session')
...
if is_main:
    ...
    swanlab.init(
        project="NeRD-Rain",
        experiment_name="DDP_2GPU_exp3_lr1e5_ft",
        config={
            **vars(args),
            "start_lr": 1e-5,
            "end_lr": 1e-6,
