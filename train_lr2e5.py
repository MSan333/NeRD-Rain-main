"""
train_lr2e5.py — 基于 DDP_4GPU_exp3_lr5e5 best checkpoint 的降 LR 续训版本（lr=2e-5）

修改点：
  - session: DDP_2GPU_exp3_lr2e5（新目录保存）
  - start_lr: 2e-5
  - end_lr:   1e-6
  - warmup:   关闭（续训不需要 warmup）
  - RESUME:   True，从 DDP_4GPU_exp3_lr5e5 的 model_best.pth 续训
  - num_epochs: 从 checkpoint epoch 起再跑 200 epoch
  - Resume 路径硬编码指向 DDP_4GPU_exp3_lr5e5 的 best checkpoint
"""
...
parser.add_argument('--session', default='DDP_2GPU_exp3_lr2e5', type=str, help='session')
...
if is_main:
    ...
    swanlab.init(
        project="NeRD-Rain",
        experiment_name="DDP_2GPU_exp3_lr2e5",
        config={
            **vars(args),
            "start_lr": 2e-5,
            "end_lr": 1e-6,
            ...
            "resume_from": "DDP_4GPU_exp3_lr5e5/model_best.pth",
        },
    )
...
# 降 LR 续训: 2e-5 → 1e-6（从 DDP_4GPU_exp3_lr5e5 best checkpoint 出发）
start_lr = 2e-5
end_lr = 1e-6
...
if RESUME:
    ...
    # 动态计算 num_epochs：从 checkpoint epoch 起再跑 200 epoch
    num_epochs = start_epoch + 200 - 1
