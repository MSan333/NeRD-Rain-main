"""
exp4: SFStar 模块训练（从头开始）
- 创新点2替换为 SFStar（空频星型交互模块）
- 2卡 DDP, lr=2e-4 → 1e-6, warmup 3 epoch, 500 epoch
- 从头训练（模块结构变化，不恢复 exp3 的 checkpoint）
"""
import os
...
parser = argparse.ArgumentParser(description='Image Deraininig - exp4 SFStar')
...
parser.add_argument('--pretrain_weights', default='./ckpt/Deraininig/models/SFStar_2GPU_lr2e4/model_latest.pth', type=str, help='Path to pretrain-weights')
parser.add_argument('--mode', default='Deraininig', type=str)
parser.add_argument('--session', default='SFStar_2GPU_lr2e4', type=str, help='session')
...
if is_main:
    ...
    swanlab.init(
        project="NeRD-Rain",
        experiment_name="SFStar_2GPU_lr2e4",
        config={
            **vars(args),
            "start_lr": 2e-4,
            "end_lr": 1e-6,
            ...
            "description": "exp4: SFStar替代MFGCP，从头训练500epoch",
        },
    )
...
if is_main:
    ...
    print('------------------------------------------------------------------------------')
    print(f"==> exp4: SFStar model, resume from epoch {start_epoch}")
    print(f"==> Current LR: {current_lr:.6f} (schedule: {start_lr} → {end_lr}, CosineAnnealing)")
