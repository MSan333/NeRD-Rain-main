"""
train_lr1e4.py — 基于 exp3 best checkpoint 的降 LR 续训版本

修改点：
  - session: DDP_2GPU_exp3_lr1e4（新目录保存）
  - start_lr: 4e-4 → 1e-4
  - end_lr:   4e-6 → 1e-6
  - warmup:   关闭（续训不需要 warmup）
  - RESUME:   True，从 exp3 的 model_best.pth 续训
  - num_epochs: 从 checkpoint epoch 起再跑 500 epoch
  - Resume 路径硬编码指向原 session DDP_4GPU_exp3 的 best checkpoint
"""
...
parser.add_argument('--session', default='DDP_2GPU_exp3_lr1e4', type=str, help='session')
...
if is_main:
    ...
    swanlab.init(
        project="NeRD-Rain",
        experiment_name="DDP_2GPU_exp3_lr1e4",
        config={
            **vars(args),
            "start_lr": 1e-4,
            "end_lr": 1e-6,
            ...
            "resume_from": "DDP_4GPU_exp3/model_best.pth",
        },
    )
...
RESUME = True
RESUME_CKPT_PATH = os.path.join(args.model_save_dir, mode, 'models', 'DDP_4GPU_exp3', 'model_best.pth')
