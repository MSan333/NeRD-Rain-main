...
### 4.1 主要消融实验（Rain200L 数据集）

| 实验编号 | 配置 | Attention | FFN | RFM | MDPConv | 预期 PSNR |
|---------|------|-----------|-----|-----|---------|----------|
| A | Baseline（原始 TransformerBlock） | ✓ | ✓ | ✗ | ✗ | 41.13 |
| B | + RFM（现有模型） | ✓ | ✓ | ✓ | ✗ | 41.55 |
| C | + MDPConv only（无 RFM） | ✓ | ✓ | ✗ | ✓ | ~41.3–41.4 |
| **D** | **+ RFM + MDPConv（MFGCP 完整）** | **✓** | **✓** | **✓** | **✓** | **~41.8–42.0** |
